...
def face_embedding(face_to_verify):
...
register_button = " "
is_register_button_successfully_pressed = False
# Main loop
while True:
    ret, frame = cap.read()
    face_objs = []
    ...
    key = cv2.waitKey(1)
    if key == ord(register_button) and len(face_objs) == 1 and not is_register_button_successfully_pressed:
        is_register_button_successfully_pressed = True
        face_embedding(face_objs[0][0].copy())
    if key == ord("q"):
        break
